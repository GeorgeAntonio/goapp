...
export function isValidPlan(
  ...
  const result = plan.reduce((currentState, action) => {
    if (match(action.precond, currentState)) {
      return next(currentState, action);
    }
    return currentState;
  }, initial);

  return match(goal, result); // Verifica se o resultado final do plano corresponde ao objetivo
}
